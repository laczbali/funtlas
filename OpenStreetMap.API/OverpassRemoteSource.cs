...
{
    internal class OverpassRemoteSource
    {
        private readonly OverpassApiConfig config;

        internal OverpassRemoteSource(OverpassApiConfig config)
        {
            this.config = config;
        }

        internal async Task<(List<Node> nodes, List<Way> selectedWays, List<Way> connectingWays)> DownloadAllDataOfArea(Area boundingBox, WayRank wayRanks)
        {
        ...
        internal async Task<List<Way>> DownloadWaysOfArea(Area boundingBox, WayRank wayRanks)
        {
        ...
        internal async Task<List<Way>> DownloadWaysOfNodes(IEnumerable<long> nodeIds)
        {
            var query = $@"
            [out:json];
            (
        ...
        internal async Task<List<Node>> DownloadNodesById(IEnumerable<long> nodeIds)
        {
            var query = $@"
            [out:json];
            (
